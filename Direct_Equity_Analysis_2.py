...
def create_enhanced_investment_report(equity_df):
    ...
    ('BOTTOMPADDING', (0, 0), (-1, -1), 5)],))
        ...
        # New Stock Performance Metrics Table
        ...
        for col in ['3M Return (%)', '6M Return (%)', '1Y Return (%)', '2Y Annualized Return (%)']:
            performance_df[col] = pd.to_numeric(performance_df[col], errors='coerce').round(2)
        ...
        perf_table_data = [perf_header] + perf_data_rows
        performance_table = Table(perf_table_data, colWidths=[2.5*inch, 1.2*inch, 1.2*inch, 1.2*inch, 1.2*inch])
...
if uploaded_file is not None:
    ...
    portfolio_df['Last Price'] = portfolio_df['Stock Code'].apply(lambda code: yf.Ticker(code).history(period="5d")['Close'].iloc[-1])
